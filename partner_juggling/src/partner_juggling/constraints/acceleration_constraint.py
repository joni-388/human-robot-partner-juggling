...
class AccelerationConstraint(TrajectoryConstraint):
    """
    ...
    def __init__(self,
        ...
        self.ddq_min = ddq_min
        self.k = k if isinstance(k, Iterable) else [k]


        constraint_name = "acc_constraint"
    ...
    def build(self, sym_pin_model: sym_pin.Model, sym_pin_data: sym_pin.Data,
        ...
        for k in self.k:
            step_name = f"ddq_constraint_{k}"

            cddq = ctraj.cddq[k,:].T
            
            for i in range(len(self.ddq_min)):
                if i == 1:
                    continue
                
                cons += inequalityConstraint(expression=self.ddq_min[i] - cddq[i], upper_bound=0, source_name=step_name + f"_min_joint_{i}", source_object=self)
                cons += inequalityConstraint(expression=cddq[i] - self.ddq_max[i], upper_bound=0, source_name=step_name + f"_max_joint_{i}", source_object=self)

            # cons += inequalityConstraint(expression=self.ddq_min - cddq, upper_bound=0, source_name=step_name + "_min", source_object=self)
            # cons += inequalityConstraint(expression=cddq - self.ddq_max, upper_bound=0, source_name=step_name + "_max", source_object=self)
